...
import { api, clearSession, fetchSecureRecord, getAccessToken, storeSession } from "@/lib/api";
...
type View = "HOME" | "MY DATA" | "PERMISSIONS" | "SECURITY" | "GATEWAY";
const FORCE_LOGIN_KEY = "samvid_force_login";
type Permission = { id: number; org: string; record: string; orgId?: string | number; recordId?: string | number; purpose: string; scope: string; expires: string; status: "ACTIVE" | "REVOKED" };
...
function asArray(value: unknown): any[] {
  ...
  if (value && typeof value === "object") {
    ...
    for (const key of ["items", "records", "consents", "data", "results"]) if (Array.isArray(record[key])) return record[key] as any[];
  }
  return [];
}
...
function normalizePermissions(value: unknown): Permission[] {
  return asArray(value).map((item, index) => {
    ...
    return { id: Number(row.id ?? row.consent_id ?? index + 1), org: String(row.organization_name ?? row.organization ?? row.org ?? row.organization_id ?? ""), record: String(row.record_title ?? row.record_name ?? row.record ?? row.record_id ?? ""), orgId: row.organization_id, recordId: row.record_id, purpose: String(row.purpose ?? ""), scope: String(row.access_type ?? row.scope ?? ""), expires: row.expiry_time ? new Date(Number(row.expiry_time) * 1000).toLocaleDateString() : "Not available", status: status === "REVOKED" || status === "REJECTED" ? "REVOKED" : "ACTIVE" };
  });
}
  ...
  return <main className="dashboard-aurora relative min-h-[calc(100vh-73px)] overflow-hidden text-[#172554]">
    ...
    <div className="pointer-events-none absolute inset-0 z-10">
      <div className="pointer-events-auto absolute right-5 top-5 flex items-center gap-2 md:right-9 md:top-7">
        ...
        {active && !collapsing && !connection && <button onClick={activateConnection} className="rounded-full border border-[#22D3EE]/35 bg-[#111447]/75 px-3 py-2 text-[9px] font-bold uppercase tracking-[.18em] text-[#2563EB] backdrop-blur hover:bg-[#14312f]">Create consent</button>}
        {connection && !collapsing && <button onClick={revokeConnection} className="rounded-full border border-[#EC4899]/40 bg-[#32114A]/75 px-3 py-2 text-[9px] font-bold uppercase tracking-[.18em] text-[#EC4899] backdrop-blur hover:bg-[#31141f]">Revoke consent</button>}
      </div>
...
function TopNav({ view, setView, userName, onLogout }: { view: View; setView: (view: View) => void; userName?: string | null; onLogout: () => void | Promise<void> }) {
  ...
  return <header className="light-glass-nav relative z-[70] flex items-center justify-between border-b px-5 py-4 md:px-9">
    ...
    <nav className="hidden items-center gap-3 md:flex">{(["HOME", "MY DATA", "PERMISSIONS", "SECURITY"] as View[]).map(item => { const Icon = item === "HOME" ? HomeIcon : item === "MY DATA" ? FileKey2 : item === "PERMISSIONS" ? UsersRound : ShieldCheck; return <button key={item} onClick={() => setView(item)} aria-current={view === item ? "page" : undefined} className={`icon-interactive flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold tracking-[.1em] ${view === item ? "text-[#4F46E5]" : "text-[#64748B]"}`}><Icon className="h-4 w-4 shrink-0" />{item}</button>; })}</nav>
    <div className="flex items-center gap-3">
      <button onClick={() => setView("GATEWAY")} className="aurora-outline-cta hidden rounded-full px-4 py-2 text-xs font-bold uppercase tracking-[.1em] text-[#3730A3] hover:text-[#1D4ED8] sm:block">Organization gateway</button>
      <div className="relative z-[200]">
        ...
        {profileOpen && <div className="pointer-events-auto absolute right-0 top-[calc(100%+10px)] z-[999] w-56 rounded-2xl border border-[#CBD5E1] bg-white p-2 shadow-2xl backdrop-blur-xl"><div className="border-b border-white/10 px-3 py-2"><p className="text-[9px] uppercase tracking-[.16em] text-[#64748B]">Signed in as</p><p className="mt-1 truncate text-xs text-[#172554]">{userName || "Profile"}</p></div><a href="/?signed_out=1" onClick={event => { event.stopPropagation(); setProfileOpen(false); void onLogout(); }} className="relative z-[1000] mt-2 flex min-h-11 w-full cursor-pointer items-center gap-2 rounded-xl bg-[#FFF1F2] px-3 py-2.5 text-left text-[10px] font-bold uppercase tracking-[.14em] text-[#BE123C] hover:bg-[#FFE4E6] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#F43F5E]" aria-label="Sign out"><LogOut className="h-3.5 w-3.5" /> Sign out</a></div>}
      </div>
    </div>
  </header>;
}

function HomeView({ active, setActive, records, permissions, pendingRequests, setView, transitionKey, userName }: { active: boolean; setActive: (value: boolean) => void; records: DataRecord[]; permissions: Permission[]; pendingRequests: number; setView: (view: View) => void; transitionKey: number; userName?: string | null }) {
  const [connection, setConnection] = useState(false);
  const [breaking, setBreaking] = useState(false);
  const [collapsing, setCollapsing] = useState(false);
...
function DataView({ records, setRecords }: { records: typeof initialRecords; setRecords: (records: typeof initialRecords) => void }) {
  ...
  const openRecord = async (record: (typeof initialRecords)[number]) => {
    ...
    try {
      if (!record.fileUrl) {
        ...
        setViewingRecord({ ...record, fileUrl: blobUrl, mimeType, fileName: record.fileName || record.title });
        return;
      }
      setViewingRecord(record);
    } catch (cause) {
      setRecordError(cause instanceof Error ? cause.message : "Unable to open this secure record.");
    }
  };

  return <main className="mx-auto max-w-7xl px-5 py-7 md:px-9 md:py-10"><div className="contents"><div className="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="eyebrow">Private vault / 02</p><h1 className="display mt-3 text-5xl font-semibold text-white">MY DATA</h1><p className="mt-3 max-w-xl text-sm leading-6 text-[#B8B0E8]">Records are encrypted, timestamped, and held under your authorization.</p></div><button onClick={() => setShowUpload(!showUpload)} className="rounded-full bg-[#22D3EE] px-5 py-3 text-[10px] font-bold uppercase tracking-[.18em] text-[#080B2A] transition hover:bg-[#66e6d5]"><UploadCloud className="mr-2 inline h-4 w-4" /> Add record</button></div>{showUpload && <div className="panel mt-7 rounded-3xl p-5 md:p-7"><div className="flex items-start justify-between"><div><p className="eyebrow">New record intake</p><h2 className="display mt-2 text-2xl text-white">Upload new record.</h2></div><button onClick={() => setShowUpload(false)} className="text-[#768791]"><X /></button></div><div className="mt-6 grid gap-4 md:grid-cols-[1fr_1fr_auto]"><input value={recordTitle} onChange={e => setRecordTitle(e.target.value)} className="data-input rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-[#172554] outline-none placeholder:text-slate-500 focus:border-[#8B5CF6]" placeholder="Record name" /><select value={recordType} onChange={e => setRecordType(e.target.value)} className="data-input rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-[#172554]"><option value="">Select type</option><option value="Identity">Identity</option><option value="Academic">Academic</option><option value="Employment">Employment</option><option value="Medical">Medical</option></select><label className="data-input flex items-center rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-[#475569]"><input type="file" onChange={e => { const file = e.target.files?.[0] ?? null; setSelectedFile(file); if (file && !recordTitle) setRecordTitle(file.name.replace(/\.[^/.]+$/, "")); }} className="max-w-full text-xs text-[#475569] file:mr-3 file:rounded-lg file:border-0 file:bg-[#EEF2FF] file:px-3 file:py-2 file:font-semibold file:text-[#4F46E5]" /></label><button disabled={!selectedFile} onClick={saveRecord} className="rounded-xl bg-[#22D3EE] px-4 py-3 text-[10px] font-bold uppercase tracking-[.14em] text-[#080B2A]">Save Record</button></div>{recordError && <p role="alert" className="mt-3 text-xs text-[#BE123C]">{recordError}</p>}</div>}<div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">{records.map(record => <div key={record.id} className="panel group rounded-3xl p-5 transition hover:-translate-y-1 hover:border-white/25"><div className="flex items-start justify-between"><div className="grid h-12 w-12 place-items-center rounded-2xl" style={{ background: `${record.accent}18`, color: record.accent }}><FileKey2 className="h-5 w-5" /></div><span className="rounded-full border border-[#22D3EE]/25 bg-[#22D3EE]/10 px-2 py-1 text-[9px] font-bold tracking-[.16em] text-[#22D3EE]">{record.verified ? "VERIFIED" : "PENDING"}</span></div><h3 className="mt-8 text-lg font-medium text-[#172554]">{record.title}</h3><p className="mt-1 text-xs text-[#64748B]">{record.type} · {record.sensitivity} sensitivity</p><div className="mt-6 grid grid-cols-2 gap-3 border-t border-slate-200 pt-4"><div><p className="eyebrow">Last accessed</p><p className="mt-2 text-xs text-[#475569]">{record.accessed}</p></div><div><p className="eyebrow">Storage</p><p className="mt-2 text-xs text-[#475569]">Encrypted</p></div></div><div className="mt-5 grid grid-cols-3 gap-2"><button onClick={() => void openRecord(record)} className="record-action col-span-2 flex items-center justify-center gap-2 rounded-xl border border-[#8B5CF6]/30 bg-[#EEF2FF] px-3 py-2 text-[10px] font-bold uppercase tracking-[.12em] text-[#4F46E5]" title="View secure record"><Activity className="h-4 w-4" /> View record</button><button disabled className="record-action cursor-not-allowed rounded-xl border border-slate-200 bg-white px-2 py-2 text-[10px] font-bold text-[#94A3B8]" title="Record editing is not exposed by the backend" aria-label={`Edit ${record.title}`}><FileKey2 className="mx-auto h-4 w-4" /></button><button disabled className="record-delete col-span-3 cursor-not-allowed rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[10px] font-bold uppercase tracking-[.12em] text-slate-400" title="Record deletion is not exposed by the backend">Remove record</button></div></div>)}</div>{viewingRecord && <div className="fixed inset-0 z-50 grid place-items-center bg-[#172554]/25 p-5 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Secure record details"><div className="w-full max-w-md rounded-3xl border border-white/70 bg-white p-6 shadow-2xl"><div className="flex items-start justify-between"><div><p className="eyebrow">Secure record</p><h2 className="display mt-2 text-2xl text-[#172554]">{viewingRecord.title}</h2></div><button onClick={() => setViewingRecord(null)} className="rounded-full p-2 text-[#475569] hover:bg-[#EEF2FF]" aria-label="Close record details"><X className="h-5 w-5" /></button></div><div className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-[#F8FAFC]">{viewingRecord.fileUrl ? (viewingRecord.mimeType?.startsWith("image/") ? <img src={viewingRecord.fileUrl} alt={viewingRecord.title} className="max-h-64 w-full object-contain" /> : viewingRecord.mimeType === "application/pdf" ? <iframe title={viewingRecord.title} src={viewingRecord.fileUrl} className="h-64 w-full" /> : <div className="p-5 text-sm text-[#475569]">Preview is not available for this file type. Use the open button below.</div>) : <div className="p-5 text-sm text-[#64748B]">No local preview is attached to this record.</div>}</div><div className="mt-4 flex items-center justify-between gap-3"><div className="text-xs text-[#475569]">{viewingRecord.fileName || "Managed record"}</div>{viewingRecord.fileUrl && <a href={viewingRecord.fileUrl} target="_blank" rel="noreferrer" className="rounded-lg bg-[#EEF2FF] px-3 py-2 text-[10px] font-bold uppercase tracking-[.12em] text-[#4F46E5]">Open document</a>}</div><div className="mt-5 grid grid-cols-2 gap-3 text-sm text-[#475569]"><div className="rounded-2xl bg-[#F8FAFC] p-4"><span className="eyebrow">Category</span><p className="mt-2 text-[#172554]">{viewingRecord.type}</p></div><div className="rounded-2xl bg-[#F8FAFC] p-4"><span className="eyebrow">Sensitivity</span><p className="mt-2 text-[#172554]">{viewingRecord.sensitivity}</p></div><div className="rounded-2xl bg-[#F8FAFC] p-4"><span className="eyebrow">Access</span><p className="mt-2 text-[#172554]">{viewingRecord.accessed}</p></div><div className="rounded-2xl bg-[#F8FAFC] p-4"><span className="eyebrow">Storage</span><p className="mt-2 text-[#172554]">Encrypted</p></div></div><div className="mt-6 grid gap-3 sm:grid-cols-2"><span className="flex cursor-not-allowed items-center justify-center rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-center text-[10px] font-bold uppercase tracking-[.12em] text-slate-400" title="Record replacement is not exposed by the backend">Replace document</span><button disabled className="cursor-not-allowed rounded-xl bg-slate-100 px-4 py-3 text-[10px] font-bold uppercase tracking-[.16em] text-slate-400" title="Verification status is supplied by the backend">{viewingRecord.verified ? "Verified" : "Backend status"}</button></div><button disabled className="mt-3 w-full cursor-not-allowed rounded-xl bg-slate-100 px-4 py-3 text-[10px] font-bold uppercase tracking-[.16em] text-slate-400" title="Record editing is not exposed by the backend">Edit title</button></div></div>}</div></main>;
}

function PermissionsView({ permissions, setPermissions, records, organizations, pendingRequests }: { permissions: Permission[]; setPermissions: (permissions: Permission[]) => void; records: DataRecord[]; organizations: Organization[]; pendingRequests: number }) {
  ...
  const revoke = async (id: number) => {
    ...
    try {
      ...
      setPermissions(permissions.map(p => p.id === id ? { ...p, status: "REVOKED", expires: "Revoked" } : p));
    } catch (cause) {
      setPermissionError(cause instanceof Error ? cause.message : "Unable to revoke this consent.");
    }
  };
  const grant = async () => {
    setPermissionError("");
    if (!selectedRecord || !purpose.trim()) {
      setPermissionError("Select a backend record and enter the access purpose.");
      return;
    }
    try {
      const request = await api.createAccessRequest({ record_id: selectedRecord, purpose: purpose.trim(), requested_access_type: accessType });
      const created = normalizePermissions([request])[0];
      if (!created) throw new Error("The backend did not return the created access request.");
      setPermissions([created, ...permissions]);
      setShowCreate(false);
      setPurpose("");
    } catch (cause) {
      setPermissionError(cause instanceof Error ? cause.message : "Unable to create this consent request.");
    }
  };
  return <main className="mx-auto max-w-6xl px-5 py-7 md:px-9 md:py-10"><div className="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="eyebrow">Consent layer / 03</p><h1 className="display mt-3 text-5xl font-semibold text-[#172554]">PERMISSIONS</h1><p className="mt-3 text-sm text-[#64748B]">Who can see your data, for what purpose, and for how long.</p></div><button onClick={() => setShowCreate(!showCreate)} className="rounded-full bg-[#22D3EE] px-5 py-3 text-[10px] font-bold uppercase tracking-[.18em] text-[#080B2A]"><Plus className="mr-2 inline h-4 w-4" /> Create permission</button></div><div className="mt-8 grid gap-3 md:grid-cols-3"><div className="panel rounded-2xl p-5"><p className="eyebrow">Active</p><p className="mt-3 text-3xl text-[#172554]">{permissions.filter(p => p.status === "ACTIVE").length.toString().padStart(2, "0")}</p></div><div className="panel rounded-2xl p-5"><p className="eyebrow">Pending requests</p><p className="mt-3 text-3xl text-[#8B5CF6]">{pendingRequests.toString().padStart(2, "0")}</p></div><div className="panel rounded-2xl p-5"><p className="eyebrow">Revoked</p><p className="mt-3 text-3xl text-[#EC4899]">{permissions.filter(p => p.status === "REVOKED").length.toString().padStart(2, "0")}</p></div></div>{permissionError && <p role="alert" className="mt-4 text-xs text-[#FCA5A5]">{permissionError}</p>}{showCreate && <div className="panel mt-7 rounded-3xl p-6"><p className="eyebrow">Guided permission</p><h2 className="display mt-2 text-2xl text-[#172554]">Define the boundary.</h2><div className="mt-6 grid gap-3 md:grid-cols-4"><select value={selectedOrganization} onChange={event => setSelectedOrganization(event.target.value)} className="data-input rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-[#172554]"><option value="">Select organization</option>{organizations.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select value={selectedRecord} onChange={event => setSelectedRecord(event.target.value)} className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-[#172554] placeholder:text-slate-500"><option value="">Select record</option>{records.map(item => <option key={item.id} value={item.id}>{item.title}</option>)}</select><input value={purpose} onChange={event => setPurpose(event.target.value)} placeholder="Purpose" className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-[#172554] placeholder:text-slate-500 placeholder:text-[#71818b]" /><button onClick={() => void grant()} className="rounded-xl border border-[#22D3EE]/40 px-4 py-3 text-[10px] font-bold uppercase tracking-[.16em] text-[#2563EB]">Grant access</button></div></div>}<div className="mt-8 space-y-3">{permissions.map(permission => <div key={permission.id} className={`panel flex flex-col gap-5 rounded-2xl p-5 md:flex-row md:items-center md:justify-between ${permission.status === "REVOKED" ? "opacity-60" : ""}`}><div className="flex items-center gap-4"><div className={`grid h-11 w-11 place-items-center rounded-2xl ${permission.status === "ACTIVE" ? "bg-[#22D3EE]/10 text-[#22D3EE]" : "bg-[#EC4899]/10 text-[#EC4899]"}`}><UsersRound className="h-5 w-5" /></div><div><p className="text-base font-medium text-[#172554]">{permission.org}</p><p className="mt-1 text-xs text-[#64748B]">{permission.record} <span className="mx-1 text-[#53616b]">/</span> {permission.purpose}</p></div></div><div className="grid grid-cols-3 gap-5 text-right md:min-w-[380px]"><div><p className="eyebrow">Scope</p><p className="mt-2 text-xs text-[#475569]">{permission.scope}</p></div><div><p className="eyebrow">Expires</p><p className="mt-2 text-xs text-[#475569]">{permission.expires}</p></div><div>{permission.status === "ACTIVE" ? <button onClick={() => revoke(permission.id)} className="rounded-full border border-[#EC4899]/30 px-3 py-2 text-[9px] font-bold uppercase tracking-[.14em] text-[#F472B6] hover:bg-[#EC4899]/10">Revoke</button> : <span className="inline-flex items-center gap-1 text-[9px] font-bold uppercase tracking-[.14em] text-[#EC4899]"><X className="h-3 w-3" /> Revoked</span>}</div></div></div>)}</div></main>;
}

function SecurityView({ permissions }: { permissions: Permission[] }) {
  const [auditEvents, setAuditEvents] = useState<string[]>([]);
  useEffect(() => {
    api.getAuditLogs().then(value => {
  ...
  return <main className="mx-auto max-w-6xl px-5 py-7 md:px-9 md:py-10"><p className="eyebrow">Integrity monitor / 04</p><h1 className="display mt-3 text-5xl font-semibold text-white">SECURITY</h1><p className="mt-3 max-w-xl text-sm leading-6 text-[#B8B0E8]">A readable history of what happened to your data, with verification states that make the underlying system legible.</p><div className="mt-8 grid gap-5 lg:grid-cols-[1.15fr_.85fr]"><section className="panel rounded-3xl p-6"><div className="flex items-center justify-between"><div><p className="eyebrow">Recent activity</p><h2 className="display mt-2 text-2xl text-white">Audit history</h2></div><Activity className="h-5 w-5 text-[#22D3EE]" /></div><div className="mt-7 space-y-5">{events.length ? events.map((event, i) => <div key={i} className="flex gap-4"><div className="flex flex-col items-center"><span className="mt-1 h-2.5 w-2.5 rounded-full bg-[#22D3EE]" /><span className="mt-2 h-full w-px bg-white/10" /></div><div className="pb-4"><p className="text-sm text-[#dce7eb]">{event}</p><p className="mt-1 flex items-center gap-1 text-[10px] uppercase tracking-[.13em] text-[#71818b]"><Clock3 className="h-3 w-3" /> Live backend audit event</p></div></div>) : <p className="rounded-2xl border border-white/10 bg-white/[.03] p-4 text-sm text-[#aebdc4]">No audit events have been returned by the backend yet.</p>}</div></section><section className="space-y-5"><div className="panel rounded-3xl p-6"><p className="eyebrow">Blockchain verification</p><div className="mt-5 space-y-4"><div className="flex items-center justify-between border-b border-white/10 pb-4"><span className="text-sm text-[#bccad0]">Consent records verified</span><span className="flex items-center gap-2 text-xs text-[#22D3EE]"><Check className="h-4 w-4" /> {permissions.length ? 'Backend data loaded' : 'No data'}</span></div><div className="flex items-center justify-between border-b border-white/10 pb-4"><span className="text-sm text-[#bccad0]">Audit history intact</span><span className="flex items-center gap-2 text-xs text-[#22D3EE]"><Check className="h-4 w-4" /> {events.length ? 'Live' : 'Awaiting data'}</span></div><div className="flex items-center justify-between"><span className="text-sm text-[#bccad0]">Contract connected</span><span className="flex items-center gap-2 text-xs text-[#8B5CF6]"><LockKeyhole className="h-4 w-4" /> {events.length ? 'Connected' : 'Unavailable'}</span></div></div></div><div className="panel rounded-3xl p-6"><p className="eyebrow">Contract security check</p><h2 className="display mt-2 text-2xl text-white">Overall risk: <span className="text-[#22D3EE]">{events.length ? 'Reported by audit' : 'Not available'}</span></h2><div className="mt-5 space-y-3"><p className="rounded-xl bg-white/[.03] px-4 py-3 text-xs text-[#aebdc4]">Security findings are shown only when returned by the backend audit service.</p></div></div></section></div></main>;
}
function GatewayView({ permissions, records, organizations, loading, dataError }: { permissions: Permission[]; records: DataRecord[]; organizations: Organization[]; loading: boolean; dataError: string }) {
  ...
  useEffect(() => {
    if (!orgId && organizations[0]) setOrgId(String(organizations[0].id));
    if (!recordId && records[0]) setRecordId(String(records[0].id));
  }, [orgId, recordId, organizations, records]);
  ...
  const checkPermission = async () => {
    ...
    setChecked(false);
    try {
      const { response, mimeType } = await fetchSecureRecord(selectedRecord.id);
      const blob = await response.blob();
      setSecureViewUrl(URL.createObjectURL(new Blob([blob], { type: mimeType })));
      setChecked(true);
    } catch (cause) {
      setChecked(true);
      setGatewayError(cause instanceof Error ? cause.message : "The backend could not authorize this record.");
    } finally {
      setChecking(false);
    }
  };
  const openSecureView = () => {
    if (secureViewUrl) window.open(secureViewUrl, "_blank", "noopener,noreferrer");
  };
  const selectionReady = Boolean(orgId && recordId && organization && selectedRecord);
  return <main className="mx-auto flex min-h-[calc(100vh-73px)] max-w-5xl items-center px-5 py-10"><div className="grid w-full gap-8 lg:grid-cols-[.8fr_1.2fr]"><section><p className="eyebrow">External organization gateway</p><h1 className="display mt-4 text-5xl font-semibold text-[#172554]">Request access<br /><span className="text-[#8B5CF6]">without overreach.</span></h1><p className="mt-5 max-w-md text-sm leading-7 text-[#B8B0E8]">A separate protected flow for organizations. SAMVID checks the owner’s permission before any secure view can open.</p><div className="mt-8 flex items-center gap-3 text-xs text-[#a6b8c1]"><LockKeyhole className="h-4 w-4 text-[#22D3EE]" /> Permission-gated by contract logic</div></section><section className="panel rounded-3xl p-6 md:p-8"><div className="flex items-center justify-between"><div><p className="eyebrow">Access gateway</p><h2 className="display mt-2 text-2xl text-white">Verify an authorized view</h2></div><span className="rounded-full border border-[#3B82F6]/30 bg-[#3B82F6]/10 px-3 py-1 text-[9px] font-bold tracking-[.14em] text-[#a9caff]">ORG-SESSION</span></div>{loading ? <div className="mt-7 space-y-3 text-sm text-[#B8B0E8]"><p>Loading organizations...</p><p>Loading records...</p></div> : <><div className="mt-7 space-y-4"><label className="block"><span className="eyebrow">Organization</span><select value={orgId} onChange={e => { setOrgId(e.target.value); setChecked(false); setGatewayError(""); }} className="data-input mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-[#172554]"><option value="">{organizations.length ? "Select organization" : "No organizations available"}</option>{organizations.map(item => <option key={item.id} value={String(item.id)}>{item.name}</option>)}</select></label><label className="block"><span className="eyebrow">Requested record</span><select value={recordId} onChange={e => { setRecordId(e.target.value); setChecked(false); setGatewayError(""); }} className="data-input mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-[#172554]"><option value="">{records.length ? "Select record" : "No records available"}</option>{records.map(item => <option key={item.id} value={String(item.id)}>{item.title}</option>)}</select></label><button onClick={() => void checkPermission()} disabled={!selectionReady || checking} className="mt-2 w-full rounded-xl bg-[#3B82F6] px-4 py-3 text-[10px] font-bold uppercase tracking-[.18em] text-white hover:bg-[#5b9aff] disabled:cursor-not-allowed disabled:opacity-50">{checking ? "Checking backend authorization..." : "Check permission"}</button>{dataError && <p role="alert" className="text-xs text-[#FCA5A5]">{dataError}</p>}</div>{checked && <div className={`mt-6 rounded-2xl border p-5 ${secureViewUrl ? "border-[#22D3EE]/30 bg-[#22D3EE]/[.07]" : "border-[#EC4899]/30 bg-[#EC4899]/[.07]"}`}>{secureViewUrl ? <><div className="flex items-center gap-2 text-[#22D3EE]"><Check className="h-5 w-5" /><span className="text-sm font-semibold">Access authorized by backend</span></div><p className="mt-3 text-xs leading-6 text-[#b4c8cd]">The selected record passed the live authorization check for {organization?.name ?? "the selected organization"}.</p><button onClick={openSecureView} className="mt-4 rounded-full border border-[#22D3EE]/40 px-4 py-2 text-[10px] font-bold uppercase tracking-[.15em] text-[#2563EB]">Open secure view</button></> : <><div className="flex items-center gap-2 text-[#EC4899]"><X className="h-5 w-5" /><span className="text-sm font-semibold">Access denied</span></div><p className="mt-3 text-xs leading-6 text-[#d1adb4]">{gatewayError || "The backend did not authorize this record."}</p></>}</div>}</>}{!loading && !organizations.length && !records.length && !dataError && <p className="mt-6 rounded-2xl border border-white/10 bg-white/[.03] p-4 text-sm text-[#aebdc4]">No organizations or records available.</p>}</section></div></main>;
}
...
export default function Home() {
  ...
  const [entry, setEntry] = useState<"LANDING" | "LOGIN" | "APP">(() => {
    ...
    return window.localStorage.getItem(FORCE_LOGIN_KEY) === "1" ? "LOGIN" : getAccessToken() ? "APP" : "LANDING";
  });
  const [backendSession, setBackendSession] = useState(() => window.localStorage.getItem(FORCE_LOGIN_KEY) !== "1" && Boolean(getAccessToken()));
  ...
  useEffect(() => {
    ...
    if ((user || backendSession) && entry !== "APP") {
      setEntry("APP");
      setView("HOME");
    } else if (!user && !backendSession && entry === "APP") {
      setEntry("LANDING");
      setView("HOME");
    }
  ...
  useEffect(() => {
    ...
    setGatewayDataError("");
    Promise.allSettled([api.me(), api.listRecords(), api.getConsents(), api.getOrganizations(), api.getAccessRequests()]).then(results => {
      if (cancelled) return;
      const [meResult, recordsResult, consentsResult, organizationsResult, requestsResult] = results;
      if (meResult.status === "fulfilled") {
        const name = meResult.value?.name ?? meResult.value?.full_name ?? meResult.value?.email;
        if (name) setBackendUserName(String(name));
      }
      if (recordsResult.status === "fulfilled") setRecords(normalizeRecords(recordsResult.value));
      if (consentsResult.status === "fulfilled") setPermissions(normalizePermissions(consentsResult.value));
      if (organizationsResult.status === "fulfilled") setOrganizations(normalizeOrganizations(organizationsResult.value));
      if (requestsResult.status === "fulfilled") {
        const requestRows = asArray(requestsResult.value);
        setPendingRequests(requestRows.filter(row => String(row.status ?? row.state ?? "").toUpperCase() === "PENDING").length);
      }
      if (organizationsResult.status === "rejected" || recordsResult.status === "rejected") {
        ...
        setGatewayDataError(failure instanceof Error ? failure.message : "Unable to load live organizations or records.");
      }
      setGatewayLoading(false);
      // A role-gated 403 is shown by the relevant view; it must not hide unrelated successful API data.
    });
  ...
  const handleLogout = () => {
    // SAMVID uses a JWT stored locally. Signing out must clear that token
    // and reset the UI immediately, without depending on the old Manus/tRPC auth server.
    window.localStorage.setItem(FORCE_LOGIN_KEY, "1");
    clearSession();
    ...
    // Remove stale signed_out query parameters and force a clean React reload.
    window.history.replaceState({}, "", window.location.pathname);
  ...
  const content = useMemo(() => {
    if (view === "MY DATA") return <DataView records={records} setRecords={setRecords} />;
    if (view === "PERMISSIONS") return <PermissionsView permissions={permissions} setPermissions={setPermissions} records={records} organizations={organizations} pendingRequests={pendingRequests} />;
    if (view === "SECURITY") return <SecurityView permissions={permissions} />;
    if (view === "GATEWAY") return <GatewayView permissions={permissions} records={records} organizations={organizations} loading={gatewayLoading} dataError={gatewayDataError} />;
    return <HomeView active={active} setActive={setActive} records={records} permissions={permissions} pendingRequests={pendingRequests} setView={setView} transitionKey={viewPulse} userName={displayName} />;
  }, [view, records, permissions, organizations, active, viewPulse, displayName, gatewayLoading, gatewayDataError]);
  ...
  return <div className="samvid-shell dashboard-shell"><AmbientIdentityField intensity={view === "HOME" ? .85 : view === "MY DATA" ? .6 : view === "PERMISSIONS" ? .55 : view === "SECURITY" ? .5 : .65} variant={view === "HOME" ? "home" : view === "MY DATA" ? "data" : view === "PERMISSIONS" ? "permissions" : view === "SECURITY" ? "security" : "gateway"} /><TopNav view={view} setView={setView} userName={displayName} onLogout={handleLogout} /><div key={view} className="view-transition">{content}</div><footer className="sticky bottom-0 z-[80] flex items-center justify-between border-t border-[#CBD5E1] bg-white/95 px-5 py-4 text-[10px] uppercase tracking-[.16em] text-[#64748B] shadow-[0_-8px_24px_rgba(100,116,139,.12)] backdrop-blur-xl md:px-9"><span>© 2026 SAMVID PROTOCOL</span></footer></div>;
}
