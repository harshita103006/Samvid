...
const start = source.indexOf("function GatewayView(");
const end = source.indexOf("\nfunction RoleSidebar", start);
if (start < 0 || end < 0) throw new Error("GatewayView boundaries not found");
const replacement = `function GatewayView({ permissions, records, organizations, loading, dataError, backendUserRole, onOpenPermissions }: { permissions: Permission[]; records: DataRecord[]; organizations: Organization[]; loading: boolean; dataError: string; backendUserRole?: string; onOpenPermissions: () => void }) {
